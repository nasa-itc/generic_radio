...
int32_t GENERIC_RADIO_SetConfiguration(socket_info_t *device, uint32_t config) {
  ...
  /* Write command */
  status = socket_send(device, write_data, GENERIC_RADIO_DEVICE_CMD_SIZE,
                       &bytes_sent, GENERIC_RADIO_CFG_DEVICE_IP,
...
** Proximity Forward
*/
int32_t GENERIC_RADIO_ProximityForward(socket_info_t *device, uint16_t scid,
  ...
  /* Write command */
  status = socket_send(device, data, data_len, &bytes_sent,
                       GENERIC_RADIO_CFG_DEVICE_IP,
...
*/
int32_t GENERIC_RADIO_RequestHK(socket_info_t *device,
                                GENERIC_RADIO_Device_HK_tlm_t *data) {
  int32_t status = OS_SUCCESS;
  uint8_t write_data[GENERIC_RADIO_DEVICE_CMD_SIZE] = {0};
  ...
  /* Write command */
  status = socket_send(device, write_data, GENERIC_RADIO_DEVICE_CMD_SIZE,
                       &bytes, GENERIC_RADIO_CFG_DEVICE_IP,
                       GENERIC_RADIO_CFG_UDP_FSW_TO_RADIO);
...
// OS_printf("\n");
      ...
      if ((read_data[0] == GENERIC_RADIO_DEVICE_HDR_0) &&
          ...
          (read_data[GENERIC_RADIO_DEVICE_HK_SIZE - 2] ==
           GENERIC_RADIO_DEVICE_TRAILER_0) &&
          (read_data[GENERIC_RADIO_DEVICE_HK_SIZE - 1] ==
           GENERIC_RADIO_DEVICE_TRAILER_1)) {
...
#ifdef GENERIC_RADIO_CFG_DEBUG
        OS_printf(
            "GENERIC_RADIO_RequestHK: Invalid header / trailer detected! \n");
#endif
        status = OS_ERROR;
      }
    }
