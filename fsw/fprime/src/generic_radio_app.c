  ...
  while (CFE_ES_RunLoop(&GENERIC_RADIO_AppData.RunStatus) == true) {
    ...
    */
    status = CFE_SB_ReceiveBuffer(
        (CFE_SB_Buffer_t **)&GENERIC_RADIO_AppData.MsgPtr,
        GENERIC_RADIO_AppData.CmdPipe, CFE_SB_PEND_FOREVER);
...
int32 GENERIC_RADIO_AppInit(void) {
  ...
  */
  ...
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_RADIO: Error registering for event services: 0x%08X\n",
        (unsigned int)status);
    return status;
  }
  ...
  */
  status = CFE_SB_CreatePipe(&GENERIC_RADIO_AppData.CmdPipe,
                             GENERIC_RADIO_PIPE_DEPTH, "RADIO_CMD_PIPE");
  if (status != CFE_SUCCESS) {
    CFE_EVS_SendEvent(GENERIC_RADIO_PIPE_ERR_EID, CFE_EVS_EventType_ERROR,
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_RADIO_CMD_MID),
                            GENERIC_RADIO_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  status = CFE_SB_Subscribe(CFE_SB_ValueToMsgId(GENERIC_RADIO_REQ_HK_MID),
                            GENERIC_RADIO_AppData.CmdPipe);
  if (status != CFE_SUCCESS) {
  ...
  */
  ...
  GENERIC_RADIO_AppData.RadioSocket.port_num =
      GENERIC_RADIO_CFG_UDP_RADIO_TO_FSW;
  ...
  GENERIC_RADIO_AppData.ProxySocket.port_num =
      GENERIC_RADIO_CFG_UDP_PROX_TO_FSW;
  ...
  if (status != SOCKET_SUCCESS) {
    CFE_EVS_SendEvent(GENERIC_RADIO_PROX_OPEN_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_RADIO: Proximity interface create error %d",
                      status);
    return status;
  }
  ...
  */
  status = CFE_ES_CreateChildTask(
      &GENERIC_RADIO_AppData.DeviceID, GENERIC_RADIO_DEVICE_NAME,
      GENERIC_RADIO_ProxyTask, 0, GENERIC_RADIO_DEVICE_STACK_SIZE,
  ...
  status = CFE_EVS_SendEvent(
      GENERIC_RADIO_STARTUP_INF_EID, CFE_EVS_EventType_INFORMATION,
      "GENERIC_RADIO App Initialized. Version %d.%d.%d.%d",
      GENERIC_RADIO_MAJOR_VERSION, GENERIC_RADIO_MINOR_VERSION,
      GENERIC_RADIO_REVISION, GENERIC_RADIO_MISSION_REV);
  if (status != CFE_SUCCESS) {
    CFE_ES_WriteToSysLog(
        "GENERIC_RADIO: Error sending initialization event: 0x%08X\n",
        (unsigned int)status);
  }
  return status;
}
...
void GENERIC_RADIO_ProcessCommandPacket(void) {
  ...
  */
  default:
    ...
    CFE_EVS_SendEvent(GENERIC_RADIO_PROCESS_CMD_ERR_EID,
                      CFE_EVS_EventType_ERROR,
                      "GENERIC_RADIO: Invalid command packet, MID = 0x%x",
...
void GENERIC_RADIO_ProcessGroundCommand(void) {
  ...
  case GENERIC_RADIO_NOOP_CC:
    ...
    */
    if (GENERIC_RADIO_VerifyCmdLength(GENERIC_RADIO_AppData.MsgPtr,
                                      sizeof(GENERIC_RADIO_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      /* Second, send EVS event on successful receipt ground commands*/
      CFE_EVS_SendEvent(GENERIC_RADIO_CMD_NOOP_INF_EID,
  ...
  */
  case GENERIC_RADIO_RESET_COUNTERS_CC:
    if (GENERIC_RADIO_VerifyCmdLength(GENERIC_RADIO_AppData.MsgPtr,
                                      sizeof(GENERIC_RADIO_NoArgs_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_RADIO_CMD_RESET_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
  ...
  */
  case GENERIC_RADIO_CONFIG_CC:
    if (GENERIC_RADIO_VerifyCmdLength(GENERIC_RADIO_AppData.MsgPtr,
                                      sizeof(GENERIC_RADIO_Config_cmd_t)) ==
        OS_SUCCESS) {
      CFE_EVS_SendEvent(GENERIC_RADIO_CMD_CONFIG_INF_EID,
                        CFE_EVS_EventType_INFORMATION,
                        "GENERIC_RADIO: Configuration command received");
      /* Command device to send HK */
      status = GENERIC_RADIO_SetConfiguration(
          &GENERIC_RADIO_AppData.RadioSocket,
          ((GENERIC_RADIO_Config_cmd_t *)GENERIC_RADIO_AppData.MsgPtr)
              ->DeviceCfg);
  ...
  */
  case GENERIC_RADIO_PROXIMITY_CC:
    CFE_MSG_GetSize((CFE_MSG_Message_t *)((GENERIC_RADIO_Proximity_cmd_t *)
                                              GENERIC_RADIO_AppData.MsgPtr)
                        ->Payload,
                    &prox_size);
    status = GENERIC_RADIO_ProximityForward(
        ...
        ((GENERIC_RADIO_Proximity_cmd_t *)GENERIC_RADIO_AppData.MsgPtr)
            ->Payload,
        prox_size);
    if (status == OS_SUCCESS) {
...
void GENERIC_RADIO_ReportHousekeeping(void) {
  ...
  status = GENERIC_RADIO_RequestHK(
      &GENERIC_RADIO_AppData.RadioSocket,
      (GENERIC_RADIO_Device_HK_tlm_t *)&GENERIC_RADIO_AppData.HkTelemetryPkt
          .DeviceHK);
  if (status == OS_SUCCESS) {
    GENERIC_RADIO_AppData.HkTelemetryPkt.DeviceCount++;
  } else {
    ...
    CFE_EVS_SendEvent(GENERIC_RADIO_REQ_HK_ERR_EID, CFE_EVS_EventType_ERROR,
                      "GENERIC_RADIO: Request device HK reported error %d",
                      status);
  }

  /* Time stamp and publish housekeeping telemetry */
  CFE_SB_TimeStampMsg(
      (CFE_MSG_Message_t *)&GENERIC_RADIO_AppData.HkTelemetryPkt);
  CFE_SB_TransmitMsg((CFE_MSG_Message_t *)&GENERIC_RADIO_AppData.HkTelemetryPkt,
                     true);
  return;
}
...
*/
int32 GENERIC_RADIO_VerifyCmdLength(CFE_MSG_Message_t *msg,
  ...
  if (expected_length == actual_length) {
    ...
    CFE_EVS_SendEvent(
        GENERIC_RADIO_LEN_ERR_EID, CFE_EVS_EventType_ERROR,
...
void GENERIC_RADIO_ProxyTask(void) {
  ...
  CFE_EVS_SendEvent(GENERIC_RADIO_TASK_REG_INF_EID,
                    CFE_EVS_EventType_INFORMATION,
  ...
  */
  while (CFE_ES_RunLoop(&GENERIC_RADIO_AppData.RunStatus) == true) {
    ...
    /* Read */
    status = socket_recv(&GENERIC_RADIO_AppData.ProxySocket, read_data,
                         sizeof(read_data), &bytes);

    if (status != SOCKET_TRY_AGAIN) {
#ifdef GENERIC_RADIO_CFG_DEBUG
      OS_printf("GENERIC_RADIO_ProxyTask reported status %d and received[%d]: ",
                status, bytes);
