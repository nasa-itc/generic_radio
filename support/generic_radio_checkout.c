...
int process_command(int cc, int num_tokens, char *tokens) {
  ...
  case CMD_NOOP:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      status = GENERIC_RADIO_CommandDevice(Generic_radioUart.handle,
                                           GENERIC_RADIO_DEVICE_NOOP_CMD, 0);
  ...
  case CMD_HK:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      status =
          GENERIC_RADIO_RequestHK(Generic_radioUart.handle, &Generic_radioHK);
      if (status == OS_SUCCESS) {
  ...
  case CMD_GENERIC_RADIO:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      status = GENERIC_RADIO_RequestData(Generic_radioUart.handle,
                                         &Generic_radioData);
      if (status == OS_SUCCESS) {
        OS_printf("GENERIC_RADIO_RequestHK command success\n");
      } else {
  ...
  case CMD_CFG:
    if (check_number_arguments(num_tokens, 1) == OS_SUCCESS) {
      ...
      status = GENERIC_RADIO_CommandDevice(
          Generic_radioUart.handle, GENERIC_RADIO_DEVICE_CFG_CMD, config);
      if (status == OS_SUCCESS) {
